...
export default function TN({movie, onClick} : {movie: {id: number, title: string, year: number, category: string, rating: string, thumbnail: {regular: {large: string}}, isTrending: boolean, isBookmarked: boolean}, index: number, onClick: Function}) {
    ...
    return (
        <div className="flex-col md:w-1/3 xl:w-1/4  w-1/2 mb-2 " >
            <div key={movie.id} className={`w-100%  mr-3 h-24 sm:h-48 md:h-40 lg:h-52 bg-cover bg-center rounded-lg relative hover:grayscale`} style={{backgroundImage:`url(${movie.thumbnail.regular.large})`}}>
            ...
            <div className="flex sm:mb-1">
                ...
                <div className="w-3.5 h-3.5 ml-1 flex items-center mr-1">
                    {movie.category === 'Movie' ? 
                        <img src="public/assets/icon-nav-movies.svg" className="mt-3 sm:mt-2" />
                    ...
                    movie.category === 'TV Series' ?
                        <img src="public/assets/icon-nav-tv-series.svg" className="mt-3 sm:mt-2" />
